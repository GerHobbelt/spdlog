...
public:
    ...
    rotating_file_sink(filename_t base_filename,
                       std::size_t max_size,
                       std::size_t max_files,
                       filename_t compress_extension,
                       std::function<void(filename_t)> compress_callback,
                       bool rotate_on_open = false,
                       const file_event_handlers &event_handlers = {});
...
std::shared_ptr<logger> rotating_logger_mt(const std::string &logger_name,
                                           ...
                                           size_t max_file_size,
                                           size_t max_files,
                                           const filename_t &compress_extension,
                                           std::function<void(filename_t)> compress_callback,
                                           bool rotate_on_open = false,
                                           const file_event_handlers &event_handlers = {}) {
    return Factory::template create<sinks::rotating_file_sink_mt>(
        logger_name, filename, max_file_size, max_files, compress_extension,
        std::move(compress_callback), rotate_on_open, event_handlers);
}
