...
SPDLOG_INLINE std::shared_ptr<logger> get(std::string_view name) SPDLOG_COND_NOEXCEPT {
    return details::registry::instance().get(name);
}

SPDLOG_INLINE std::shared_ptr<logger> get(const char *name) SPDLOG_COND_NOEXCEPT {
    return details::registry::instance().get(name);
}
#endif
